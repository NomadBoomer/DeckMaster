...
const CostEstimatorCard: React.FC<CostEstimatorCardProps> = ({ estimate, loading, zipCode }) => {
  const formatValue = (val: string) => {
    // Remove decimals if present in currency strings
    return val.replace(/\.\d{2}/, '').replace(/(\$\d+)\.00/, '$1');
  };

  return (
    <div id="cost-estimator-card" className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden h-full flex flex-col">
            ...
            <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-emerald-50 p-4 rounded-lg text-center">
                        ...
                        <p className="text-2xl font-bold text-emerald-800">{formatValue(estimate.materialTotal)}</p>
                    </div>
                    <div className="bg-blue-50 p-4 rounded-lg text-center">
                        <p className="text-xs text-gray-500 uppercase font-bold">Est. Pro Labor</p>
                        <p className="text-2xl font-bold text-blue-800">{formatValue(estimate.laborTotal)}</p>
                    </div>
                     <div className="bg-amber-50 p-4 rounded-lg text-center">
                        <p className="text-xs text-gray-500 uppercase font-bold">Contingency</p>
                        <p className="text-2xl font-bold text-amber-800">{formatValue(estimate.contingency)}</p>
                    </div>
                </div>
                ...
                <div className="border-t border-gray-100 pt-4">
                    <div className="flex justify-between items-center mb-4">
                         ...
                         <div className="text-sm text-gray-600 bg-gray-100 px-3 py-1 rounded">
                             Permits Est: <span className="font-bold text-gray-900">{formatValue(estimate.permitFees)}</span>
                         </div>
                    </div>
                                    ...
                                    <>
                                        ...
                                        <tr className="bg-gray-50 border-t-2 border-gray-200">
                                            ...
                                            <td className="p-3 text-right font-extrabold text-emerald-800 text-lg">
                                                {formatValue(estimate.materialTotal)}
                                            </td>
                                        </tr>
                                    </>
                                ) : (
                                    <tr>
                                        <td colSpan={4} className="p-3 text-center text-gray-400 italic">Detailed breakdown unavailable.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                ...
                {estimate.sources.length > 0 && (
                    <div className="bg-gray-50 p-4 rounded border border-gray-200 mt-auto no-print">
                        <h5 className="text-xs font-bold text-gray-500 uppercase mb-2">Live Data Sources</h5>
